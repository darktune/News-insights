"use client";

import Link from "next/link";
import AchihiLogo from "./AchihiLogo";
import { Mail, MapPin } from "lucide-react";
import { CATEGORIES } from "@/lib/mock-data";

export default function Footer() {
  return (
    <footer className="mt-16 border-t border-[var(--border)] bg-[var(--bg-secondary)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8 mb-10">
          
          {/* Brand & Mission Column */}
          <div className="sm:col-span-2 md:col-span-2 space-y-4">
            <AchihiLogo size={40} className="mb-2" />
            <p className="text-sm leading-relaxed max-w-sm text-[var(--text-secondary)]">
              ACHICHI Media Limited is an independent Nigerian digital news and intelligence organization.
              Rooted in truth, driven by the public interest.
            </p>

            <div className="flex items-center gap-2 text-xs text-[var(--text-tertiary)] pt-1">
              <MapPin size={13} className="text-[var(--accent)] flex-shrink-0" />
              <span>Victoria Island, Lagos · CBD, Abuja</span>
            </div>

            {/* Social Icons */}
            <div className="flex items-center gap-3 pt-2">
              {[
                { 
                  icon: <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>, 
                  label: "Twitter / X", 
                  href: "https://x.com" 
                },
                { 
                  icon: <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>, 
                  label: "Facebook", 
                  href: "https://facebook.com" 
                },
                { 
                  icon: <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.88z"/></svg>, 
                  label: "Instagram", 
                  href: "https://instagram.com" 
                },
                { 
                  icon: <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.5 12 3.5 12 3.5s-7.505 0-9.377.55a3.015 3.015 0 0 0-2.122 2.136C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.55 9.376.55 9.376.55s7.505 0 9.377-.55a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>, 
                  label: "YouTube", 
                  href: "https://youtube.com" 
                },
                { 
                  icon: <Mail size={14} />, 
                  label: "Email Newsroom", 
                  href: "mailto:contact@achichimedia.com" 
                }
              ].map((item, i) => (
                <a 
                  key={i} 
                  href={item.href} 
                  target={item.href.startsWith("http") ? "_blank" : undefined}
                  rel={item.href.startsWith("http") ? "noopener noreferrer" : undefined}
                  aria-label={item.label}
                  className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold transition-all bg-[var(--tag-bg)] text-[var(--text-secondary)] border border-[var(--border)] hover:bg-[var(--accent)] hover:text-[var(--bg)]"
                >
                  {item.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Column 2: News Categories */}
          <div>
            <h4 className="font-black text-xs uppercase tracking-widest mb-4 text-[var(--text)]">
              News Desks
            </h4>
            <ul className="space-y-2.5">
              {CATEGORIES.map((cat) => (
                <li key={cat}>
                  <Link 
                    href={`/category/${cat.toLowerCase()}`}
                    className="text-sm transition-colors text-[var(--text-secondary)] hover:text-[var(--accent)] font-medium"
                  >
                    {cat}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 3: Company & Editorial */}
          <div>
            <h4 className="font-black text-xs uppercase tracking-widest mb-4 text-[var(--text)]">
              Company
            </h4>
            <ul className="space-y-2.5">
              {[
                { label: "About Us", href: "/about" },
                { label: "Newsroom Staff", href: "/about#leadership" },
                { label: "Careers", href: "/careers" },
                { label: "Contact Us", href: "/contact" },
                { label: "Whistleblower Tips", href: "/contact" },
              ].map(({ label, href }) => (
                <li key={label}>
                  <Link 
                    href={href}
                    className="text-sm transition-colors text-[var(--text-secondary)] hover:text-[var(--accent)] font-medium"
                  >
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 4: Services & Legal */}
          <div>
            <h4 className="font-black text-xs uppercase tracking-widest mb-4 text-[var(--text)]">
              Services & Legal
            </h4>
            <ul className="space-y-2.5">
              {[
                { label: "Advertise with Us", href: "/advertise" },
                { label: "Write for Us (Op-Eds)", href: "/write-for-us" },
                { label: "Morning Briefing", href: "/#newsletter" },
                { label: "Privacy Policy", href: "/privacy" },
                { label: "Terms of Service", href: "/terms" },
              ].map(({ label, href }) => (
                <li key={label}>
                  <Link 
                    href={href}
                    className="text-sm transition-colors text-[var(--text-secondary)] hover:text-[var(--accent)] font-medium"
                  >
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

        </div>

        {/* Bottom Bar: Copyright, RC registration, Lagos */}
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-[var(--border)]">
          <p className="text-xs text-[var(--text-tertiary)]">
            © {new Date().getFullYear()} ACHICHI Media Limited. All rights reserved.
          </p>
          <p className="text-xs text-[var(--text-tertiary)]">
            RC: 1789420 · Victoria Island, Lagos & Maitama, Abuja, Nigeria
          </p>
        </div>

      </div>
    </footer>
  );
}
